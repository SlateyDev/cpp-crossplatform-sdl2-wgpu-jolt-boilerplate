...
bool NavMeshRuntime::Build(const std::vector<float> &vertices, const std::vector<int> &indices)
{
    ...
    rcMarkWalkableTriangles(&context, config.walkableSlopeAngle, vertices.data(), vertexCount, indices.data(), triangleCount, triangleAreas.data());
    if (!rcRasterizeTriangles(&context, vertices.data(), vertexCount, indices.data(), triangleAreas.data(), triangleCount, *heightField, config.walkableClimb)) {
        freeHeightField();
    ...
    if (!dtCreateNavMeshData(&params, &navData, &navDataSize)) {
        freeDetailMesh();
        freePolyMesh();
        status = "NavMesh: Detour data creation failed";
        return false;
    }
